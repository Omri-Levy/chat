...
class StoreClient {
    ...
    async findMessages(room) {
        try {
            const messages = (await this.redisStore.lrange(`message:${room}`, 0, 10));
    ...
    async createMessage(room, message) {
        try {
            const messagesLen = await this.redisStore.rpush(`message:${room}`, JSON.stringify(message));
            if (messagesLen > 10) {
    ...
    async createUser(room, user) {
        try {
            await this.redisStore.hset(`user:${room}`, user.username, JSON.stringify(user));
            return [];
        }
        catch {
    ...
    async deleteUser(room, username) {
        try {
            ...
            return [
                undefined,
                new Error(`Failed to delete user:${room} with username ${username}..`),
            ];
        }
    }
    async findUser(room, username) {
        try {
            ...
            return [
                undefined,
                new Error(`Failed to find user:${room} with username ${username}..`),
            ];
        }
    }
